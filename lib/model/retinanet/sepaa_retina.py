...
class SEPAA_retinanet(nn.Module):

    def __init__(self, num_classes, block, layers, attention_type, reduce_dim, beta, 
    ...
    def attention_module(self, fpn_q_feats, fpn_s_feats):
        ...
        for p in range(len(fpn_q_feats)):
            ...
            for i in range(self.num_shot):
                ...
                s_feat = s_feat + self.beta * support_channel_global
...
def SEPAARetinaNet(num_classes, attention_type, reduce_dim, beta, num_way=2, num_shot=5, pos_encoding=True, pretrained=False):
    """Constructs a ResNet-50 model.
    Args:
    """
    model = SEPAA_retinanet(num_classes, Bottleneck, [3, 4, 6, 3], attention_type, reduce_dim, beta,
