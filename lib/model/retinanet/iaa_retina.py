...
class IAA_retinanet(nn.Module):
    ...
    def __init__(self, num_classes, block, layers, attention_type='concat', reduce_dim=400, gamma=0.1, 
        ...
        super(IAA_retinanet, self).__init__()
    ...
    def attention_module(self, fpn_q_feats, fpn_s_feats):
        ...
        for p in range(len(fpn_q_feats)):
            ...
            # query channel enhance
            query_spatial_weight = self.channel_K_layers[p](q_feat)  # [B, h*w, 1]
            query_spatial_weight = F.softmax(query_spatial_weight, 1)
            query_channel_global = torch.bmm(query_spatial_weight.transpose(1, 2), q_feat)  # [B, 1, c]
            q_feat = q_feat + self.gamma * query_channel_global
            ...
            for i in range(self.num_shot):
                ...
                s_feat = s_feat + self.gamma * support_channel_global
                ...
                support_adaptive_attention_weight = support_adaptive_attention_weight + self.gamma * unary_term.transpose(1, 2)  # [B, hw, nn]
...
def IAARetinaNet(num_classes, attention_type, reduce_dim, gamma, num_way=2, num_shot=5, pos_encoding=True, pretrained=False):
    """Constructs a ResNet-50 model.
    Args:
    """
    model = IAA_retinanet(num_classes, Bottleneck, [3, 4, 6, 3], attention_type=attention_type, reduce_dim=reduce_dim, gamma=gamma,
