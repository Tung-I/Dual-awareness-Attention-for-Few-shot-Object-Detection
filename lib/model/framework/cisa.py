...
class _CISARCNN(nn.Module):
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes, support_ims):
        if self.training:
        ...
        for i in range(self.n_shot):
            ...
            ############################################################################################################
            # heat_mat_to_save = support_adaptive_attention_weight .view(1, feat_h, feat_w, 20, 20).squeeze().detach().cpu().numpy()
            # np.save('/home/tony/Desktop/vrep_image/heat_map.npy', heat_mat_to_save)
            # np.save('/home/tony/Desktop/vrep_image/sup_data.npy', support_ims.squeeze().permute(1, 2, 0).detach().cpu().numpy())
            # np.save('/home/tony/Desktop/vrep_image/im_data.npy', im_data.squeeze().permute(1, 2, 0).detach().cpu().numpy())
            # print('support attention saved')
            # raise Exception(' ')
            ############################################################################################################
    ...
    def rcnn_head(self, pooled_feat, support_feat):
        ...
        cls_prob = F.softmax(cls_score, 1)  # [B*128, 2]
...
class CISARCNN(_CISARCNN):
    ...
    def finetune(self):
        ...
        self.output_score_layer = nn.Linear(64* 49, 2)
        self.RCNN_bbox_pred = nn.Linear(2048, 4)
        self.RCNN_bbox_pred.weight.data.normal_(0, 0.001)
        self.RCNN_bbox_pred.bias.data.zero_()
        self.output_score_layer.weight.data.normal_(0, 0.01)
        self.output_score_layer.bias.data.zero_()
        ...
        for param in self.rpn_unary_layer.parameters():
            param.requires_grad = True
        for param in self.rcnn_unary_layer.parameters():
            param.requires_grad = True
