...
class _fgnRCNN(nn.Module):
    """ faster RCNN """
    def __init__(self, classes, n_way=2, n_shot=5):
        super(_fgnRCNN, self).__init__()
        ...
        # few shot rcnn head
        self.avgpool_rpn = nn.AvgPool2d(20)
        self.avgpool_rcnn = nn.AvgPool2d(14, stride=1)
        self.cls_conv1 = nn.Conv2d(2048, 512, 3, padding=0, bias=False)
        self.bn1 = nn.BatchNorm2d(512)
        self.cls_conv2 = nn.Conv2d(512, 128, 3, padding=0, bias=False)
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes, support_ims, all_cls_gt_boxes=None):
        ...
        if self.training:
            ...
            pos_feat_rpn = self.avgpool_rpn(pos_support_feat)
            pos_feat_rcnn = self.avgpool_rcnn(pos_support_feat)
            neg_feat_rcnn = self.avgpool_rcnn(neg_support_feat)
        else:
            support_ims = support_ims.view(-1, support_ims.size(2),  support_ims.size(3),  support_ims.size(4))
            support_feats = self.RCNN_base(support_ims)
        ...
        # attention RPN
        batch_size = pos_feat_rpn.size(0)
        feat_before_rpn = []
        for _vec, _feat in zip(pos_feat_rpn.chunk(batch_size, dim=0), base_feat.chunk(batch_size, dim=0)):
            feat_before_rpn += [_feat * _vec]
        feat_before_rpn = torch.cat(feat_before_rpn)  # [B, c, h, w]
        ## rois [B, RPN_POST_NMS_TOP_N(2000), 5], 5 is [batch_num, x1, y1, x2, y2]
        rois, rpn_loss_cls, rpn_loss_bbox = self.RCNN_rpn(feat_before_rpn, im_info, gt_boxes, num_boxes)
        ...
        # rcnn head
        if self.training:
            bbox_pred, cls_prob, cls_score_all = self.rcnn_head(pooled_feat, pos_feat_rcnn)
            _, neg_cls_prob, neg_cls_score_all = self.rcnn_head(pooled_feat, neg_feat_rcnn)
    ...
    def rcnn_head(self, pooled_feat, support_feat):
        # box
        bbox_pred = self.RCNN_bbox_pred(self._head_to_tail(pooled_feat))
        # cls
        combined_feat = []
        ...
        cls_feat = self.cls_conv1(combined_feat)
        cls_feat = self.bn1(cls_feat)
        cls_feat = F.relu(cls_feat)
        cls_feat = self.cls_conv2(cls_feat)
        cls_feat = self.bn2(cls_feat)
        cls_feat = F.relu(cls_feat)
        cls_score = self.RCNN_cls_score(cls_feat.view(cls_feat.size(0), -1))
...
class FGN(_fgnRCNN):
  def __init__(self, classes, num_layers=50, pretrained=False, num_way=2, num_shot=5):
    ...
    _fgnRCNN.__init__(self, classes, n_way=num_way, n_shot=num_shot)
  ...
  def _init_modules(self):
    ...
    #         nn.Linear(256, 2)
    #     )
    self.RCNN_cls_score = nn.Linear(1152, 2)
